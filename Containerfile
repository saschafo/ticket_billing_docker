...
# --- Abweichung vom offiziellen frappe_docker/images/custom/Containerfile -----
# Lokale App-Quellen. Bei APP_SOURCE=local legt build.sh hier eine Arbeitskopie
# des App-Repos ab; apps.json verweist dann auf /opt/frappe/apps-local/<app>.
# bench erkennt absolute Pfade als "on disk"-Quelle und klont von dort.
# Bei APP_SOURCE=git ist das Verzeichnis leer und ohne Wirkung.
COPY --chown=frappe:frappe apps-local /opt/frappe/apps-local
# -----------------------------------------------------------------------------
...
RUN --mount=type=secret,id=apps_json,target=/opt/frappe/apps.json,uid=1000,gid=1000 \
  ...
  # bench protokolliert den Klon-Befehl mitsamt der Adresse -- bei einem
  # privaten Repo steht der Token damit im Klartext in logs/bench.log und
  # waere fuer jeden lesbar, der das Abbild hat. Das Protokoll des Bauvorgangs
  # hat im fertigen Abbild ohnehin nichts verloren.
  rm -f logs/*.log
